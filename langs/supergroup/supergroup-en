...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return 'SuperGroup is not installed.'
  end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "demoteadmin" then
		...
		send_large_msg(channel_id, text)
	elseif get_cmd == "setleader" then
		local group_owner = data[tostring(result.to.peer_id)]['set_leader']
		if group_owner then
		local channel_id = 'channel#id'..result.to.peer_id
...
local function set_supergroup_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, '#Error: Failed, please try again!', ok_cb, false)
  end
end
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'SuperGroup is not installed.', ok_cb, false)
			end
			print("SuperGroup "..msg.to.print_name.."("..msg.to.id..") unistalled")
			superrem(msg)
			rem_mutes(msg.to.id)
		end
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "SuperGroup ID of [" ..string.gsub(msg.to.print_name, "_", " ").. "]:\n\n"..msg.to.id
			end
		end
